...
3.  **Kriging**: Predict at new locations, drawing on information from
    all sources even when each source only observes a subset of the
    parameters.
4.  **Return levels**: Estimate r-year return levels with uncertainty
    via Monte Carlo simulation and/or the delta method.

## Data

The package includes `coast_data`, a dataset of annual maximum sea
levels at 129 sites along the U.S. Gulf and Atlantic coasts: 29 NOAA
...
Fit GEV(mu, sigma, xi) at each site via maximum likelihood. Parameters
...
bootstrap (resampling years to preserve spatial dependence within each
...
Predict at new locations. The kriging predictor fuses information from
both NOAA and ADCIRC observations to produce NOAA-scale GEV parameters
...
The closed-form block LOO-CV (Rasmussen & Williams, 2006, eq. 5.12)
...
The `source_params` argument generalizes the framework beyond
NOAA/ADCIRC. Each entry maps a data source name to its parameter indices
in the joint model:
