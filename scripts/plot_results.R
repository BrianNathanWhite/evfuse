...
ggsave("figures/stage1_mle_map.png", p1, width = 16, height = 5.5, dpi = 300,
       bg = "white")
...
make_krig_panel <- function(param_col, label, scale) {
  ...
  ggplot() +
    basemap +
    geom_sf(data = pts, aes(color = value), size = 1.2, shape = 16, alpha = 0.7) +
    scale +
    map_coord() +
...
ggsave("figures/kriged_params_map.png", p2, width = 16, height = 5.5, dpi = 300,
       bg = "white")
...
p3 <- ggplot() +
  ...
  geom_sf(data = noaa_pts, aes(fill = return_level),
          shape = 21, size = 3.5, stroke = 0.6, color = "black", alpha = 0.85) +
  scale_color_viridis_c(option = "inferno", begin = 0.1, end = 0.95,
...
ggsave("figures/return_levels.png", p3_4, width = 14, height = 5.5, dpi = 300,
       bg = "white")
