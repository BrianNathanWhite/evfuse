...
tab_sig   <- tab[tab$sig, ]
tab_nosig <- tab[!tab$sig, ]

p2 <- ggplot() +
  geom_polygon(data = states_df,
               aes(x = long, y = lat, group = group),
               fill = "gray90", colour = "white", linewidth = 0.3) +
  # Non-significant: grey diamonds (no slope color)
  geom_point(data = tab_nosig, aes(x = lon, y = lat),
             shape = 23, size = 3, fill = "grey75", color = "grey45", stroke = 0.7) +
  # Significant: colored circles
  geom_point(data = tab_sig, aes(x = lon, y = lat, fill = sens_slope),
             shape = 21, size = 4, color = "black", stroke = 0.9) +
  scale_fill_gradient2(low = "#2166AC", mid = "grey90", high = "#B2182B",
                       midpoint = 0, limits = slope_lim,
                       name = "Sen's slope\n(m/year)") +
  # Significance legend via off-screen points
  geom_point(data = data.frame(x = -200, y = -200),
             aes(x = x, y = y, shape = "p < 0.05 (MK)"),
             fill = "grey50", size = 4, stroke = 0.9) +
  geom_point(data = data.frame(x = -200, y = -200),
             aes(x = x, y = y, shape = "p >= 0.05"),
             fill = "grey75", color = "grey45", size = 3, stroke = 0.7) +
  scale_shape_manual(values = c("p < 0.05 (MK)" = 21, "p >= 0.05" = 23),
                     name = NULL) +
  guides(shape = guide_legend(order = 1,
           override.aes = list(fill = c("grey50", "grey75"),
                               color = c("black", "grey45"),
                               stroke = c(0.9, 0.7), size = c(4, 3))),
