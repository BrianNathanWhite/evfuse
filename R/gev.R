...
#' covariance matrices. For a nonstationary alternative that accounts for
#' sea level trends at NOAA sites, see \code{\link{fit_gev_detrended}}.
#'
...
#'   parameters.
#' @seealso \code{\link{fit_gev_detrended}} for the nonstationary version.
...
fit_gev_all <- function(dat, log_scale = TRUE) {
  ...
  for (i in seq_len(n)) {
    ...
    if (is.null(V_raw) || any(is.na(V_raw)) || any(diag(V_raw) < 0)) {
