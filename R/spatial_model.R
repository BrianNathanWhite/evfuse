...
#' dSigma/dA_{ab} and dSigma/drho_k are rank-1 Kronecker products, enabling
...
fit_spatial_model <- function(stage1, dat, W_tap, D,
  ...
  nll_grad <- function(par) {
    ...
    # --- A gradient (Eq. 7) ---
    # dSigma/dA_{ab} = (e_a (Ae_b)^T) ⊗ Omega_b + ((Ae_a) e_b^T) ⊗ Omega_a
    # d(nll)/dA_{ab} = 0.5 * (e_a^T H_b (Ae_b) + (Ae_a)^T H_a e_b)
    #               = 0.5 * (H_b[a,:] . A[:,b]  +  A[:,a] . H_a[:,b])
    grad_A <- matrix(0, p, p)
    for (a in seq_len(p)) {
      for (b in seq_len(a)) {
        grad_A[a, b] <- 0.5 * (sum(H[[b]][a, ] * A[, b]) +
                                  sum(A[, a] * H[[a]][, b]))
      }
    }
